...
class Parser:
    ...
    @staticmethod
    def _annotate(node, token, position):
        node.location = SourceLocation(token.line, token.col, position)
        return node
    ...
    def handle_operator_arguments(self):
        ...
        if self.current_token().type != tokens.TokenType.CLOSE_PAREN:
            if self.current_token().type in [
                ...
                raise InvalidTypeError(
                    "expected a datatype", self.current_token(), self.position
                )
    ...
    def handle_function(self, public):
        ...
        if self.current_token().type in [
            ...
            raise InvalidTypeError(
                "expected a return datatype", self.current_token(), self.position
            )
    ...
    def expect(self, token):
        if self.at_end():
            raise UnexpectedEOFError(
                token,
                location=SourceLocation(position=self.position),
    ...
    def handle_map_literal(self):
        ...
        while self.current_token().type != tokens.TokenType.CLOSE_PAREN:
            ...
            if not isinstance(key, ast.Literal):
                raise InvalidMapKeyError(
                    "Map keys must be literals; use a literal value as the key",
                    self.current_token(),
                    self.position,
                )
    ...
    def handle_primary(self):
        ...
        if token.type in [
            ...
            raise InvalidExpressionError(
                "expected a literal, identifier, or parenthesised expression",
                token,
                self.position,
            )
    ...
    def handle_statement(self):
        ...
        if self.current_token().type == tokens.TokenType.IDENTIFIER:
            ...
            if self.current_token().type in [
                ...
                return self._annotate(
                    ast.Assignment(expr, operator, value), token, start
                )
        ...
        if handler is None:
            raise InvalidStatementError("expected a statement", token, self.position)
        ...
        if token.type in [
            ...
            return self._annotate(handler(self, public), token, start)
        else:
            return self._annotate(handler(self), token, start)
